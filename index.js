...
const mainMenu = (ctx) => {
    ...
    return Markup.keyboard([[msgs.buttons.student.homework, msgs.buttons.student.help]]).resize();
};
...
bot.hears(msgs.buttons.student.help, checkPrivate, checkMembership, (ctx) => {
    ...
    ctx.reply(msgs.helpPrompt, Markup.keyboard([[msgs.buttons.common.finish]]).resize());
});

bot.hears(msgs.buttons.teacher.setHomework, checkPrivate, (ctx) => {
    if (!isTeacher(ctx)) return;
    userStates[ctx.from.id] = { step: 'WAITING_HW' };
    ctx.reply(msgs.setHwPrompt, Markup.removeKeyboard());
});

bot.hears(msgs.buttons.teacher.sendNews, checkPrivate, (ctx) => {
    if (!isTeacher(ctx)) return;
...
bot.on('message', checkPrivate, checkMembership, async (ctx) => {
    ...
    if (state.step === 'STUDENT_SENDING_HELP') {
        if (ctx.message.text === msgs.buttons.common.finish) {
            ...
            return ctx.reply("✅ Ваш запрос отправлен учителю.", mainMenu(ctx));
        }
...
bot.on('callback_query', async (ctx) => {
    if (ctx.callbackQuery.data.startsWith('ans_')) {
        const studentId = ctx.callbackQuery.data.replace('ans_', '');
